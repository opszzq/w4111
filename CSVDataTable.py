...
class CSVDataTable(BaseDataTable):
    ...
    def find_by_primary_key(self, key_fields, field_list=None):
        ...
        :return: None, or a dictionary containing the requested fields for the record identified
            by the key.
        """
        key_dic = dict(zip(self._key_columns, key_fields))
        res = self.find_by_template(key_dic)
        if len(res)>0:
            return res[0]
        else:
            return None
    ...
    def delete_by_key(self, key_fields):
        ...
        for name in key_fields:
            res.append(dict(zip(self._key_columns, [name])))

        count = 0

        for each_template in res:
            count += self.delete_by_template(each_template)
        return count
    ...
    def delete_by_template(self, template):
        ...
        for r in self._rows:
            if not self.matches_template(r, template):
                modified_row.append(r)
            else:
                counter += 1
        self._rows = modified_row
    ...
    def update_by_key(self, key_fields, new_values):
        ...
        for each_template in res:
            count += self.update_by_template(each_template, new_values)
        return count
    ...
    def update_by_template(self, template, new_values):
        ...
        for r in self._rows:
            if not self.matches_template(r, template):
                ...
                m_rows = r
                m_rows.update(new_values)
                modified_row.append(m_rows)
                counter += 1
    ...
    def insert(self, new_record):
        ...
        kc = self._key_columns[0]
        count = 0
        if kc in new_record.keys():
            for r in self._rows:
                if r[kc] == new_record[kc]:
                    count += 1
